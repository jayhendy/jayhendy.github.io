 ...
 *
 * Date: Fri, 24 Aug 2018 19:55:08 GMT
 */
...
(function (global, factory) {
    ...
    var mouseDrags = function mouseDrags(rootDOM) {
      return rxjs.fromEvent(rootDOM, 'contextmenu').pipe(operators.map(function (downEvt) {
    ...
    var draw = (function (drag$, _ref) {
      var _ref$initStroke = _ref.initStroke,
          initStroke = _ref$initStroke === undefined ? [] : _ref$initStroke,
          _ref$type = _ref.type,
          type = _ref$type === undefined ? undefined : _ref$type;

      var typeOpts = type === undefined ? {} : { type: type };
    ...
    var recognizeMMStroke = function recognizeMMStroke(stroke, model) {
      var _ref2 = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {},
          ...
          maxDepth_ = _ref2$maxDepth === undefined ? model.getMaxDepth() : _ref2$maxDepth,
          _ref2$requireMenu = _ref2.requireMenu,
          requireMenu = _ref2$requireMenu === undefined ? false : _ref2$requireMenu,
          _ref2$requireLeaf = _ref2.requireLeaf,
          requireLeaf = _ref2$requireLeaf === undefined ? !requireMenu : _ref2$requireLeaf;
    ...
    var recognizeMMStrokeForConfirmation = function recognizeMMStrokeForConfirmation(stroke, model) {
      var _ref2 = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {},
          _ref2$maxDepth = _ref2.maxDepth,
          maxDepth_ = _ref2$maxDepth === undefined ? model.getMaxDepth() : _ref2$maxDepth,
          _ref2$requireMenu = _ref2.requireMenu,
      ...
      var sensitivity = 0.75;
      var angleThreshold = 360 / maxMenuBreadth / 2 / sensitivity;
      var articulationPoints = getStrokeArticulationPoints(stroke, expectedSegmentLength, angleThreshold).slice(0, 2);
      var minSegmentSize = expectedSegmentLength / 3;
      // Get the segments of the marking menus.
    ...
    var expertNavigation = (function (drag$, model) {
      ...
      var draw$ = draw(drag$, { initStroke: initStroke, type: 'draw' }).pipe(operators.share());
    ...
    var confirmedNoviceNavigationHOO = function confirmedNoviceNavigationHOO(drag$, start, model, options) {
      return dwellings(drag$, options.noviceDwellingTime, options.movementsThreshold).pipe(operators.take(1), operators.map(function () {
        return (start != null ? rxjs.of(start) : drag$).pipe(operators.take(1), operators.mergeMap(function (start_) {
          ...
          drag$.pipe(operators.skip(1)), model, _extends({}, options, { menuCenter: start_.position })).pipe(operators.map(function (n) {
    ...
    var expertToNoviceSwitchHOO = function expertToNoviceSwitchHOO(drag$, model, initStroke, options) {
      return dwellings(draw(drag$, { initStroke: initStroke }), options.noviceDwellingTime, options.movementsThreshold).pipe(operators.take(1), operators.map(function (evt) {
        ...
        if (!menu || menu.isRoot()) {
          ...
          var secondMenu = recognizeMMStrokeForConfirmation(evt.stroke, model, {
            maxDepth: -1,
            requireMenu: true
          });
    ...
    var confirmedExpertNavigationHOO = function confirmedExpertNavigationHOO(drag$, model, options) {
      return longMoves(draw(drag$, { type: 'draw' }), options.movementsThreshold).pipe(operators.take(1), operators.map(function (e) {
      ...
      }), operators.map(function (nav$) {
        return rxjs.merge(rxjs.of(nav$), expertToNoviceSwitchHOO(drag$, model, nav$.stroke, options));
      }), operators.switchAll());
    };
    ...
    var navigationFromDrag = function navigationFromDrag(drag$, start, model, options) {
      var _ref = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : {},
          _ref$confirmedExpertN = _ref.confirmedExpertNavigationHOO,
          confirmedExpertNavigationHOO_ = _ref$confirmedExpertN === undefined ? confirmedExpertNavigationHOO : _ref$confirmedExpertN,
          _ref$confirmedNoviceN = _ref.confirmedNoviceNavigationHOO,
          confirmedNoviceNavigationHOO_ = _ref$confirmedNoviceN === undefined ? confirmedNoviceNavigationHOO : _ref$confirmedNoviceN,
          _ref$startup = _ref.startup,
          startup_ = _ref$startup === undefined ? startup : _ref$startup;
    ...
    var connectLayout = (function (parentDOM, navigation$, createMenuLayout, createUpperStrokeCanvas, createLowerStrokeCanvas, createGestureFeedback, log) {
      ...
      var upperStrokeCanvas = null;
      // A stroke drawn below the menu.
      var lowerStrokeCanvas = null;
      // The points of the lower strokes.
      var lowerStroke = null;
      ...
      var showGestureFeedback = function showGestureFeedback() {
        gestureFeedback.show(lowerStroke ? [].concat(lowerStroke, upperStroke) : upperStroke);
      };
    ...
    var createGestureFeedback = (function (parentDOM, _ref) {
      var duration = _ref.duration,
          strokeOptions = objectWithoutProperties(_ref, ['duration']);

      var strokeTimeoutEntries = [];

      var show = function show(stroke) {
        var canvas = createStrokeCanvas(parentDOM, strokeOptions);
    ...
    var MMItem = function () {
       ...
       */
      function MMItem(id, name, angle$$1) {
      ...
      MMItem.prototype.isRoot = function isRoot() {
        return !this.parent;
      };
    ...
    var recursivelyCreateModelItems = function recursivelyCreateModelItems(items) {
      ...
      return Object.freeze(items.map(function (item, i) {
        ...
        // Create the item.
        var mmItem = new MMItem(item.id == null ? stdId : item.id, typeof item === 'string' ? item : item.name, i * angleRange, { parent: parent });
    ...
    var main = (function (items, parentDOM) {
      var _ref = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {},
          ...
          strokeColor = _ref$strokeColor === undefined ? '#000' : _ref$strokeColor,
          ...
          lowerStrokeColor = _ref$lowerStrokeColor === undefined ? '#777' : _ref$lowerStrokeColor,
          _ref$lowerStrokeWidth = _ref.lowerStrokeWidth,
          lowerStrokeWidth = _ref$lowerStrokeWidth === undefined ? strokeWidth : _ref$lowerStrokeWidth,
          _ref$lowerStrokeStart = _ref.lowerStrokeStartPointRadius,
          lowerStrokeStartPointRadius = _ref$lowerStrokeStart === undefined ? lowerStrokeWidth : _ref$lowerStrokeStart,
          _ref$gestureFeedbackD = _ref.gestureFeedbackDuration,
          gestureFeedbackDuration = _ref$gestureFeedbackD === undefined ? 1000 : _ref$gestureFeedbackD,
          _ref$gestureFeedbackS = _ref.gestureFeedbackStrokeWidth,
          gestureFeedbackStrokeWidth = _ref$gestureFeedbackS === undefined ? strokeWidth : _ref$gestureFeedbackS,
          _ref$gestureFeedbackS2 = _ref.gestureFeedbackStrokeColor,
          gestureFeedbackStrokeColor = _ref$gestureFeedbackS2 === undefined ? strokeColor : _ref$gestureFeedbackS2,
